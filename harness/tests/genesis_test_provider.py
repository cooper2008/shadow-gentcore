...
GENESIS_OUTPUTS: dict[str, dict[str, Any]] = {
    ...
    "BestPracticeAdvisorAgent": {
        "best_practices_md": (
            "# Best-practice overlay (backend)\n\n"
            "_Generated by BestPracticeAdvisorAgent — test fixture._\n\n"
            "Standards coverage is complete against the backend baseline "
            "(smoke fixture — no gaps reported).\n"
        ),
        "gap_summary": {
            "total_principles": 10,
            "covered": 10,
    ...
    "BestPracticeResearchAgent": {
        "knowledge_map": {
            "standards_sources": [
                {"name": "Structured logging", "principle_id": "structured_logging", "severity": "must"},
            ],
            "reference_topics": [
                {"name": "Structured logging — implementation", "principle_id": "structured_logging"},
            ],
            "workflow_processes": [
                {"name": "feature_delivery", "confidence": 1.0, "signals": ["intent", "industry:backend"]},
                {"name": "bug_fix", "confidence": 1.0, "signals": ["intent", "industry:backend"]},
            ],
            "compliance_rules": [],
            "output_templates": [],
            "glossary_terms": [],
        ...
        "coverage": {
            "standards": 60, "workflows": 70, "compliance": 20,
            "tools": 20, "roles": 10, "overall": 50,
        },
        "gaps": [
            {"category": "roles", "description": "No role signals from intent alone",
             "severity": "warning", "suggestion": "Add a `team:` field to domain.yaml to seed roles."},
        ],
        "research_notes": ["Test fixture — synthesized from backend baseline."],
    },
