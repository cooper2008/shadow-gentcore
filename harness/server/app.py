...
class RunRequest(BaseModel):
    """Auto-triaged request — one endpoint for any task.

    The domain's `workflows/triage.yaml` decides which downstream workflow
    handles the task. Use `/run/workflow` directly when you want to bypass
    triage and pick a specific workflow.
    """

    task: Any = Field(
        description="Task description (string) or a task input dict.",
    )
    domain: str = Field(default="", description="Path to domain repo (defaults to DOMAIN_PATH env)")
    dry_run: bool = Field(default=False)
...
def create_app(domain_path: str = "") -> FastAPI:
    ...
    @app.post("/run", tags=["run"], dependencies=[Depends(_check_auth)])
    async def run_triaged(req: RunRequest) -> dict[str, Any]:
        """One-call entry point — dispatches via workflows/triage.yaml.

        Consumers POST `{task: "..."}` and the domain's triage workflow
        (generated by AgentBuilderAgent when ≥3 workflows exist) classifies
        and routes to the right downstream workflow. If no triage.yaml is
        present, returns 404 with the list of available workflows so the
        caller knows to use /run/workflow explicitly.
        """
        try:
            await asyncio.wait_for(_semaphore.acquire(), timeout=0.1)
        except asyncio.TimeoutError:
        ...
        try:
            ...
            if not d:
                raise HTTPException(
                    status_code=400,
                    detail="domain path required (pass `domain` or set DOMAIN_PATH env)",
                )
            triage_path = Path(d) / "workflows" / "triage.yaml"
            if not triage_path.exists():
                workflows_dir = Path(d) / "workflows"
                available = (
                ...
                raise HTTPException(
                    status_code=404,
                    detail={
                        "message": "No triage.yaml — this domain hasn't generated the dispatcher yet.",
                        "hint": "Re-run genesis with ≥3 workflows so Builder emits triage.yaml, OR call /run/workflow directly.",
                        "available_workflows": available,
                    },
                )

            # Normalise task: accept string or dict.
            task_input = {"task": req.task} if isinstance(req.task, str) else (req.task or {})

            try:
                result = await asyncio.wait_for(
                    run_workflow(str(triage_path), task_input, d, dry_run=req.dry_run),
                    timeout=_REQUEST_TIMEOUT,
                )
            except asyncio.TimeoutError:
                raise HTTPException(status_code=504, detail="Request timed out")
