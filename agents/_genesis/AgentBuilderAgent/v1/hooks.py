...
def _propagate_credentials(domain_root: Path, written_paths: list[str]) -> dict[str, Any]:
    ...
    for p_str in written_paths:
        ...
        if not reqs:
            continue
        entries = [
            {"name": r.name, "purpose": r.purpose, "required": r.required}
            for r in reqs
        ]
        per_agent[agent_name] = entries
        for r in reqs:
            all_creds.setdefault(r.name, {"purpose": r.purpose, "required": r.required, "used_by": []})
    ...
    # Write REQUIRED_CREDENTIALS.md at domain root
    if all_creds:
        lines = [
            ...
            "",
            "| Credential | Purpose | Required | Used by |",
            "| --- | --- | --- | --- |",
        ]
        for name in sorted(all_creds):
            info = all_creds[name]
        ...
        for agent_name in sorted(per_agent):
            ...
            for e in per_agent[agent_name]:
                ...
                lines.append(f"- `{e['name']}` ({marker}) — {e['purpose']}")
            lines.append("")
        try:
